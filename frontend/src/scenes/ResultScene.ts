...
export class ResultScene extends Phaser.Scene {
  ...
  create(data: { result: any }): void {
    ...
    const emoji = this.add.text(width / 2, -60, isWinner ? '🏆' : '😔', {
      fontSize: '90px',
    }).setOrigin(0.5);
    ...
    // Win/Lose text
    const winText = this.add.text(width / 2, height * 0.28, isWinner ? LANG.win : LANG.lose, {
      fontSize: '34px',
    ...
    if (!isWinner && result.winner_name) {
      const winnerLabel = this.add.text(width / 2, height * 0.38, LANG.playerWon(result.winner_name), {
        fontSize: '18px',
        fontFamily: 'IRANSans, Vazir, Tahoma, Arial, sans-serif',
        color: '#cccccc',
      }).setOrigin(0.5).setAlpha(0);
    ...
    this.add.text(width / 2, height * 0.47, reasonText, {
      fontSize: '15px',
      fontFamily: 'IRANSans, Vazir, Tahoma, Arial, sans-serif',
      color: '#aaaaaa',
    }).setOrigin(0.5);
    ...
    // Play Again button
    const playAgainBtn = this.add.text(width / 2, height * 0.64, LANG.playAgain, {
      fontSize: '18px',
      fontFamily: 'IRANSans, Vazir, Tahoma, Arial, sans-serif',
      color: '#ffffff',
    ...
    playAgainBtn.on('pointerover', () => playAgainBtn.setBackgroundColor('#ff8c42'));
    playAgainBtn.on('pointerout', () => playAgainBtn.setBackgroundColor('#f97316'));
    playAgainBtn.on('pointerdown', () => {
      window.location.reload();
    });

    // Back to lobby button
    const lobbyBtn = this.add.text(width / 2, height * 0.76, LANG.backToLobby, {
      fontSize: '15px',
      fontFamily: 'IRANSans, Vazir, Tahoma, Arial, sans-serif',
      color: '#888888',
    }).setOrigin(0.5).setInteractive({ useHandCursor: true }).setAlpha(0);
    ...
    lobbyBtn.on('pointerdown', () => {
      if ((window as any).ReactNativeWebView) {
        (window as any).ReactNativeWebView.postMessage(JSON.stringify({ type: 'GAME_FINISHED', ...result }));
      }
      window.history.back();
    });
    ...
    if (isWinner) {
      this.time.addEvent({
        ...
        callback: () => {
          ...
          particle.fillStyle(colors[Phaser.Math.Between(0, colors.length - 1)], 0.8);
